...
student_pairs <- function(attendance, group_size = 2, population = 1000,
                          pair_history = NULL,
                          file_path = NULL){

  # convert pair_history to list if dataframe
  ...
  present_students <- attendance$name[attendance$present]

  # generate new pairings
  new_pairs <- create_pairs(pool = present_students,
...
#' @examples
#' full_class <- LETTERS[1:26]
#' attendance <- take_attendance(full_class = full_class, absent = c("B", "M", "O", "Z"))
#'
take_attendance <- function(full_class,
  ...
  attendance <- tibble::tibble(
    name = full_class,
    present = full_class %in% present_students,
    session = session_id
  )
